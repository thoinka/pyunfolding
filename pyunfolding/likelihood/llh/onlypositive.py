...
class OnlyMonotonic(LikelihoodTerm):
    """Supresses negative terms in :math:`f` as :math:`\sum_i \frac{a}{1+\exp(f_i / s)}`, whereas
    :math:`s` is a smoothness variable, and a is the penalty for negative values.
    """
    formula = ''
    ...
    def func(self, model, f, g):
        if self.exclude_edges:
            f = f[1:-1]
        if self.s == 0.0:
            return (np.diff(f) < 0.0).any() * self.a
        elif self.s == -0.0:
            return (np.diff(f) > 0.0).any() * self.a
        else:
            return np.sum(self.a / (1.0 + safe_exp(np.diff(f) / self.s)))
    ...
    """def grad(self, model, f, g):
        if self.s == 0.0 or self.s == -0.0:
            ...
            f_adj = f[1:] + f[:-1]
            exp_term = safe_exp(f / self.s)
            exp_adj = exp_term[1:] + exp_term[:-1]
            R = self.a * safe_exp(f_adj / self.s) / exp_adj ** 2 / self.s
            output = np.r_[R, 0.0] - np.r_[0.0, R]
            if self.exclude_edges:
                output[[0,-1]] = 0.0
            return output"""
...
class OnlyConcave(LikelihoodTerm):
    """Supresses negative terms in :math:`f` as :math:`\sum_i \frac{a}{1+\exp(f_i / s)}`, whereas
    :math:`s` is a smoothness variable, and a is the penalty for negative values.
    """
    formula = ''
    ...
    def func(self, model, f, g):
        if self.exclude_edges:
            f = f[1:-1]
        if self.s == 0.0:
            return (np.diff(f, 2) < 0.0).any() * self.a
        elif self.s == -0.0:
            return (np.diff(f, 2) > 0.0).any() * self.a
        else:
            return np.sum(self.a / (1.0 + safe_exp(np.diff(f, 2) / self.s)))